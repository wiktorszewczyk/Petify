...
class DonationService {
  ...
  Future<List<Donation>> getUserDonations() async {
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        return content.map((donationJson) => Donation.fromBackendJson(donationJson)).toList();
      }
  ...
  Future<Donation> addMaterialDonation({
    ...
    try {
      final donationIntentResponse = await _api.post('/donations/intent', data: {
        'shelterId': shelterId,
        'petId': petId,
  ...
  Future<Donation> addMonetaryDonation({
    ...
    try {
      ...
      final donationIntentResponse = await _api.post('/donations/intent', data: requestData);
  ...
  Future<Map<String, dynamic>> calculatePaymentFee(double amount, String provider) async {
    try {
  ...
  Future<Map<String, dynamic>> initializePayment({
    ...
    try {
      final response = await _api.post('/donations/$donationId/payment/initialize',
        data: {
          'provider': provider,
